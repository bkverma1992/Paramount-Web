...
//  Paramount
//
//  Created by Yugasalabs-28 on 28/05/2019.
...
class CurrencyConverter {
     ...
     */
    public func convertAndFormat(_ value : Double, valueCurrency : String, outputCurrency : String, numberStyle : NumberFormatter.Style, decimalPlaces : Int) -> String? {
        guard let doubleOutput = convert(value, valueCurrency: Currency(rawValue: valueCurrency)!, outputCurrency: Currency(rawValue: outputCurrency)!) else {
            return nil
        }
...
private class CurrencyXMLParser : NSObject, XMLParserDelegate {
    ...
    func newLatestCurrencyData()  {
        
        var rateDict = [String: Any]()
        rateDict = (UserDefaults.standard .value(forKey: "currencyRate") as! NSDictionary) as! [String : Any]
        print(rateDict)
     
        var totalContact = Array<Any>()
        for (key,value)  in rateDict
        {
            let ssss = NSMutableDictionary()
            
            let rollNumber:String = String(format: "%@", value as! CVarArg)
            ssss .setValue(key, forKey: "currency")
            ssss .setValue(rollNumber, forKey: "value")
            totalContact .append(ssss)
        }
        print(totalContact)
        
        for i in 0..<totalContact.count
        {
            print(totalContact[i])
            var dict = NSDictionary()
            dict = totalContact[i] as! NSDictionary
            print(dict)
            guard let currency = dict["currency"] else { return }
            guard let rate = dict["value"] else { return }
            print(rate)
            guard let exchangeRate = Paramount.makeExchangeRate(currency: currency as! String, rate: rate as! String) else { return }
...
func makeExchangeRate(currency : String, rate : String) -> (currency : Currency, rate : Double)? {
    guard let currency = Currency(rawValue: currency) else { return nil }
    guard let rate = Double(rate) else { return nil }
    return (currency, rate)
}
